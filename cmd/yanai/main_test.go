...
func approvedCycle(t *testing.T) (app, workspace string) {
	app, workspace, _ = approvedCycleWithChecks(t)
	return app, workspace
}

func approvedCycleWithChecks(t *testing.T) (app, workspace string, checks checkControl) {
	t.Helper()
	harness, app, workspace := siblings(t)
	t.Chdir(harness)
...
func TestMockCLIUsesBindingAndGuardsRun(t *testing.T) {
	...
	if st.Phase != ws.PhaseAwaitingReview {
		t.Fatalf("phase = %q, want %q", st.Phase, ws.PhaseAwaitingReview)
	}
	for _, task := range st.Tasks {
		if task.Status != workflow.TicketImplemented {
			t.Fatalf("task %s = %q, want implemented", task.ID, task.Status)
		}
		applied := filepath.Join(app, "yanai-server/ejemplo-"+task.Owner+".md")
		if _, err := os.Stat(applied); err != nil {
			t.Fatalf("approved change not applied to the checkout: %v", err)
		}
	}
	// The change lives in the application checkout, not as a copy in the
	// harness: the whole point of Step 8 is that there is exactly one of it.
	if _, err := os.Stat(filepath.Join(workspace, "cycles/001/entregables")); !os.IsNotExist(err) {
		t.Fatal("run generated an application mirror under entregables")
	}
	if err := filepath.WalkDir(filepath.Join(workspace, "cycles/001/execution"), func(path string, d os.DirEntry, err error) error {
		if err != nil || d.IsDir() {
			return err
		}
		if strings.HasSuffix(path, ".go") || strings.Contains(path, "archivos") {
			return fmt.Errorf("evidence directory holds an application copy: %s", path)
		}
		return nil
	}); err != nil {
		t.Fatal(err)
	}
	// A rerun is idempotent: nothing is regenerated, reapplied or rechecked.
	if err := cmdRun([]string{"--ws", workspace}); err != nil {
		t.Fatalf("idempotent rerun: %v", err)
	}
	if again := load(t, workspace); again.Phase != ws.PhaseAwaitingReview {
		t.Fatalf("rerun changed phase to %q", again.Phase)
	}
}
...
func TestRunRejectsDifferentCheckoutWithSameHEAD(t *testing.T) {
	...
	if err := cmdRun([]string{"--ws", workspace}); err == nil || !strings.Contains(err.Error(), "checkout identity differs") {
...
// legacyInit preserves old interview regression fixtures with an explicit target.
func legacyInit(t *testing.T, workspace string) error {
	t.Helper()
	if _, err := os.Stat(filepath.Join(workspace, "yanai.config.json")); os.IsNotExist(err) {
		if err := cmdInit([]string{"--ws", workspace, "--repo", "../yanai", "--module-dir", "yanai-server", "--allow", "AGENTS.md,yanai-server"}); err != nil {
			return err
		}
		_, _, err := templates.Extract(workspace)
		return err
	}
	return cmdInit([]string{"--ws", workspace})
}
