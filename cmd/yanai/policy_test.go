...
func configureExecution(t *testing.T, workspace string) {
	...
	doc["execution"] = workflow.ExecutionPolicy{MaxTokens: 10000000, MaxCostUSD: 100, MaxActiveSeconds: 3600, MaxCalls: 100, MaxRepairs: 10, Prices: prices, Checks: []workflow.Check{{ID: "backend-test", Args: []string{"go", "test", "./internal/example"}, Dir: "yanai-server", TimeoutSeconds: 60}}}
...
func TestEnginePatchStateAllowsOwnChangesButRejectsExternalEdits(t *testing.T) {
	...
	file := filepath.Join(app, "yanai-server/ejemplo.md")
