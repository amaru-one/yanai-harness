...
// mockCandidate answers the execution request it was actually given: it reads
// the ticket and its declared outputs back out of the message and writes one
// entry per output. A mock that hardcoded a path would pass while the real
// contract validation it is meant to exercise never ran.
func mockCandidate(message string) string {
	ticket, outputs := workflow.DeclaredOutputs(message)
	c := workflow.Candidate{
		SchemaVersion: workflow.CandidateSchemaVersion,
		Ticket:        ticket,
		Result:        workflow.CandidateChange,
		Explanation:   "Entrega simulada (YANAI_MOCK=1): escribe un archivo de ejemplo por cada salida declarada.",
	}
	for _, path := range outputs {
		source := fmt.Sprintf("# Ejemplo\n\nArchivo generado en modo simulado para %s.\n", ticket)
...
func mockTicketPlan(message string) string {
	_, rest, ok := strings.Cut(message, "\nINPUT:\n")
	if !ok {
		return "{}"
	}
	raw, _, _ := strings.Cut(rest, "\nPRIOR TURNS:")
	var input struct{ Ticket workflow.MarkdownTicket }
	if json.Unmarshal([]byte(raw), &input) != nil {
		return "{}"
	}
	var ids []string
	for _, ac := range input.Ticket.Criteria {
		ids = append(ids, ac.ID)
	}
	reply := map[string]any{"summary": "Synthetic engineer-only plan. DB architect and designer are unnecessary for this mock fixture.", "specialists": []any{}, "observations": []any{}, "tasks": []workflow.Ticket{{ID: "T-001", Title: input.Ticket.Title, Description: input.Ticket.Task, Outputs: []string{"harness-example.md"}, AllowedPaths: []string{"harness-example.md"}, Scope: ids, MaxAttempts: 2}}}
