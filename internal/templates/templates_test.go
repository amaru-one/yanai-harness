...
func TestLocalRolePromptsNeverUpgrade(t *testing.T) {
	dest := t.TempDir()
	extract(t, dest)
	for _, key := range []string{"prompts/ingeniero.md", "prompts/arquitecto-bd.md", "prompts/disenador.md"} {
		original := "old shipped prompt for " + key
		if err := os.WriteFile(filepath.Join(dest, key), []byte(original), 0600); err != nil {
			t.Fatal(err)
		}
		shipOlderVersion(t, dest, key, original)
	}
	extract(t, dest)
	for _, key := range []string{"prompts/ingeniero.md", "prompts/arquitecto-bd.md", "prompts/disenador.md"} {
		if read(t, filepath.Join(dest, key)) != "old shipped prompt for "+key {
			t.Fatal("role prompt overwritten", key)
		}
		if _, err := os.Stat(filepath.Join(dest, key) + ".new"); !os.IsNotExist(err) {
			t.Fatal("role prompt upgrade sidecar created", key)
		}
	}
}
