...
func fixture(t *testing.T) config.Repo {
	...
	return config.Repo{Path: root, ModuleDir: "yanai-server", AllowedPaths: []string{"AGENTS.md", "yanai-server"}, Extensions: []string{".go", ".md", ".json", ".mod"}, ExcludeDirs: []string{"vendor"}}
}
...
func TestRejectWrongTargetsAndWorkspaceOverlap(t *testing.T) {
	...
	if _, err := repository.Open(r, ""); err == nil {
		t.Fatal("accepted harness module as application")
	}
}
...
func TestGenericRootAndBroadFingerprint(t *testing.T) {
	r := fixture(t)
	write(t, r.Path, "go.mod", "module example.test/arbitrary\n\ngo 1.26.6\n")
	write(t, r.Path, "outside-context.md", "baseline\n")
	git(t, r.Path, "add", ".")
	git(t, r.Path, "-c", "user.name=Test", "-c", "user.email=test@example.invalid", "-c", "commit.gpgsign=false", "commit", "-qm", "root module")
	r.ModuleDir = "."
	r.AllowedPaths = []string{"."}
	target := open(t, r)
	if err := target.CheckDirectory("."); err != nil {
		t.Fatal(err)
	}
	// Context is narrowed after binding; fingerprinting must still notice changes outside it.
	r.ModuleDir = "yanai-server"
	r.AllowedPaths = []string{"yanai-server"}
	target = open(t, r)
	first, err := target.Snapshot()
	if err != nil {
		t.Fatal(err)
	}
	if _, ok := first.Content["outside-context.md"]; !ok {
		t.Fatal("fingerprint silently inherited narrow context paths")
	}
	write(t, r.Path, "outside-context.md", "first edit\n")
	second, err := target.Snapshot()
	if err != nil {
		t.Fatal(err)
	}
	write(t, r.Path, "outside-context.md", "second edit\n")
	third, err := target.Snapshot()
	if err != nil {
		t.Fatal(err)
	}
	if second.StatusHash != third.StatusHash || second.Baseline() == third.Baseline() {
		t.Fatal("same-status external edit escaped fingerprint")
	}
}
